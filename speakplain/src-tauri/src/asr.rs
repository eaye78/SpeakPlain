...
impl SenseVoiceEngine {
    ...
    fn build_session(model_path: &PathBuf, _use_gpu: bool) -> anyhow::Result<Session> {
        info!("开始创建 ONNX Session...");
        let session = Session::builder()
            .map_err(|e| anyhow::anyhow!("创建 session builder 失败: {}", e))?
            .with_intra_threads(4)
            .map_err(|e| anyhow::anyhow!("设置线程数失败: {}", e))?
            .with_optimization_level(GraphOptimizationLevel::Disable)  // 禁用优化，debug 下加载速度最快
    ...
    fn get_model_dir() -> anyhow::Result<PathBuf> {
        ...
        // 1. exe 所在目录（生产环境）
        let p = exe_dir.join("models").join("sensevoice");
        if p.exists() { return Ok(p); }

        // 2. 当前工作目录
        let p = PathBuf::from("models/sensevoice");
        if p.exists() { return Ok(p); }

        // 3. dev 模式：CARGO_MANIFEST_DIR 同级的 models 目录
        if let Ok(manifest) = std::env::var("CARGO_MANIFEST_DIR") {
            let p = PathBuf::from(&manifest).join("..")
                .join("models").join("sensevoice");
        ...
        if let Some(data_dir) = dirs::data_dir() {
            let p = data_dir.join("SpeakPlain").join("models").join("sensevoice");
            if p.exists() { return Ok(p); }
        }

        Err(anyhow::anyhow!("未找到模型目录，请将模型放到 models/sensevoice/ 下"))
    }
